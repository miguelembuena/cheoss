...
public class Zobrist implements Constants {
	...
	// random long array from polyglot
	...
	public static long calculateBoardKey(Board board) {
		//key=piece^castle^enpassant^turn;
		long key = 0L;
		
		// piece part
		for (int square=26;square<118;square++) {
			int piece = board.pieceAt(square);
			if (piece != OUT && piece != EMPTY) {
				key = key ^ piecePartOfKey(piece, square);
			}
		}
...
//		black can castle long      3
		if (board.getCsWhiteShort()!= 0) key = key ^ random64[768 + 0];
		if (board.getCsWhiteLong()!= 0) key = key ^ random64[768 + 1];
		if (board.getCsBlackShort()!= 0) key = key ^ random64[768 + 2];
		if (board.getCsBlackLong()!= 0) key = key ^ random64[768 + 3];
		
		//en passant part
		int offset =  enPassantCol(board);
		if (offset != -1) {
			key = key ^ random64[772 + offset];
		}
			
		
		//If white is to move then "turn" is the sole entry of RandomTurn. If not then "turn" is zero. 
		if (board.isWhiteTurn()) {
			key = key ^ random64[780];			
		}		
		
		return key;
	}
	
	public static long piecePartOfKey(int piece, int square) {
		//offset_piece=64*kind_of_piece+8*row+file
		int offset_piece = 64 * typeOfPiece(piece) + 8 * row(square) + col(square);
		return random64[offset_piece];
	}
	...
	public static long newKeyAfterMove(Board board, int move ) {
		long key = board.getKey();

		// esto creo que es el turno de juego, usar nombres descriptivos con constantes
		key = key ^ random64[780];
		
		key = key ^ piecePartOfKey(XMove.pieceMoved(move), XMove.from(move)); // borra de origen
		
		//1.1 si es enroque borrar y poner torre correspondiente
		if (XMove.flag(move) == FLAG_CASTLING) {
			if (XMove.equals(move, WHITE_SHORT_CASTLING)) {
				key = key ^ piecePartOfKey(WROOK, h1); // borra torre de origen
				key = key ^ piecePartOfKey(WROOK, f1); // pone torre en destino
			}
			else if (XMove.equals(move, WHITE_LONG_CASTLING)) {
				key = key ^ piecePartOfKey(WROOK, a1); // borra torre de origen
				key = key ^ piecePartOfKey(WROOK, d1); // pone torre en destino
			}
			else if (XMove.equals(move, BLACK_SHORT_CASTLING)) {
				key = key ^ piecePartOfKey(BROOK, h8); // borra torre de origen
				key = key ^ piecePartOfKey(BROOK, f8); // pone torre en destino
			}
			else if (XMove.equals(move, BLACK_LONG_CASTLING)) {
				key = key ^ piecePartOfKey(BROOK, a8); // borra torre de origen
				key = key ^ piecePartOfKey(BROOK, d8); // pone torre en destino
			}  			
		}

		if (XMove.isCapture(move)) {
			if (XMove.flag(move) == FLAG_EN_PASSANT_CAPTURE) {
				int pawnCaptured = XMove.isWhite(move)? BPAWN:WPAWN;
				int square = XMove.isWhite(move)? XMove.to(move)+12:XMove.to(move)-12;
				key = key ^ piecePartOfKey(pawnCaptured, square); // borra peon capturado al paso
			}			
			else {
				key = key ^ piecePartOfKey(XMove.pieceCaptured(move), XMove.to(move)); // borra pieza tomada	
			}			
		}
		...
		if (XMove.flag(move) >= FLAG_PROMO_QUEEN) {
			...
			case FLAG_PROMO_QUEEN:
				promo = XMove.isWhite(move)? WQUEEN:BQUEEN;
				break;
			case FLAG_PROMO_ROOK:
				promo = XMove.isWhite(move)? WROOK:BROOK;
				break;
			case FLAG_PROMO_BISHOP:
				promo = XMove.isWhite(move)? WBISHOP:BBISHOP;
				break;
			case FLAG_PROMO_KNIGHT:
				promo = XMove.isWhite(move)? WKNIGHT:BKNIGHT;
				break;
			default:
				System.out.println("wrong promo flag!!");				
			}
			key = key ^ piecePartOfKey(promo, XMove.to(move)); // pone en destino
		}
		else {
			key = key ^ piecePartOfKey(XMove.pieceMoved(move), XMove.to(move)); // pone en destino	
		}
	...
	//	If the opponent has performed a double pawn push and there is now a pawn next to it belonging 
	//to the player to move then "enpassant" is the entry from RandomEnPassant whose offset is the 
	//file of the pushed pawn (counted from 0(=a) to 7(=h)). If this does not apply then enpassant=0.
	//
	//	Note that this is different from the FEN standard. In the FEN standard the presence of an 
	//"en passant target square" after a double pawn push is unconditional.ç
	public static int enPassantCol(Board board) {
		int ep = board.getEnPassant();
		if (ep == 0) return -1;

		int eprank = (ep % 12) - 2;		
		if (board.isWhiteTurn()) {
			if ( board.pieceAt(ep + 11) == WPAWN ||
				 board.pieceAt(ep + 13) == WPAWN ) {
				return eprank;
			}
		}
		else {
			if ( board.pieceAt(ep - 11) == BPAWN ||
					 board.pieceAt(ep - 13) == BPAWN ) {
					return eprank;
				}			
		}
	...
	public static int typeOfPiece(int piece) {
		return piece%2 == 0? piece-2 : piece;
	}
